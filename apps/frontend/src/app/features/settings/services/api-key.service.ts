...
export class ApiKeyService {
  ...
  /**
   * Get headers for TTS requests.
   * When useServerKey is true, the backend will use its own server-side API key.
   */
  getTtsHeaders(
    provider: string,
    useServerKey = false,
    ...
    // If useServerKey is true, don't send any client-side API key
    // The backend will use its own server-side key from environment variables
    if (!useServerKey) {

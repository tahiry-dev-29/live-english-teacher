...
export class VadService {
  ...
  async start(callbacks: {
    ...
    try {
      ...
      this.logger.info(MESSAGES.log.vadStarted);
    } catch (error) {
      this.logger.error(MESSAGES.log.vadStartFailed, error);
      throw error;
    }
  }
  ...
  stop(): void {
    ...
    this.logger.info(MESSAGES.log.vadStopped);
  }
    ...
    if (volume > this.SPEECH_THRESHOLD) {
      if (!this.isSpeaking()) {
        if (!this.speechStartTime) {
          ...
          this.onSpeechStart?.();
          this.logger.info(MESSAGES.log.speechStarted, { volume });
        }
      }
      ...
      if (this.isSpeaking()) {
        if (
          this.lastSpeechTime &&
          now - this.lastSpeechTime > this.SPEECH_END_DELAY
        ) {
          this.isSpeaking.set(false);
          this.speechStartTime = null;
          this.onSpeechEnd?.();
          this.logger.info(MESSAGES.log.speechEnded);
        }
      } else {
        this.speechStartTime = null;
      }

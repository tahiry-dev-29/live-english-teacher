...
import { VoiceControlComponent } from '@features/voice-call/voice-control/voice-control-component';
...
export class ChatContainerComponent {
  ...
  /** True while tokens stream in — hides "Thinking…", shows live cursor. */
  ...
  /** Live bubble: last message while tokens stream in. */
  readonly streamingMessageIndex = computed<number | null>(() => {
    if (!this.streaming()) return null;
    const msgs = this.messages();
    return msgs.length > 0 ? msgs.length - 1 : null;
  });
